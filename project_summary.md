...
*   **Săn Boss MVP Xoay Vòng Map (Auto MVP Rotation)**: Tự động phát hiện Boss thế giới còn sống trên map hiện tại. Hỗ trợ cấu hình danh sách Map xoay vòng (`mvpTargetMaps`), tự động chuyển sang Map tiếp theo ngay sau khi hạ Boss hoặc theo chu kỳ giờ tròn. Hỗ trợ cấu hình nâng cao: sắp xếp ưu tiên săn theo Khoảng cách gần nhất hoặc Cấp độ (thấp/cao), lọc bỏ Boss theo Blacklist và ưu tiên theo Whitelist. Bot tự di chuyển tiếp cận mục tiêu, tiêu diệt Boss, xoay vòng Map và tự quay lại farm Zone khi hoàn thành.
    *   **Khắc phục Triệt để Lỗi Kẹt Xoay Map (2026-08-04)**:
        *   Sửa lỗi kẹt `this.bosses = null` khi map sạch boss: Tự động khởi tạo `this.bosses = []` khi nhận payload `full: 1` không có boss, giúp bot xác nhận map sạch ngay nhịp đầu tiên (1-2s) thay vì bị ngâm 80s/map.
        *   Giải quyết xung định vị Nông Trại: Thêm điều kiện `!this.isMvpCycling` khi kiểm tra Nông Trại để bot săn boss bình thường tại Map 5 ("Tàn tích Cổ đại") mà không bị lệnh `home_exit` kích ra ngoài.
        *   Reset linh hoạt `force_mvp_hunt`: Cho phép nút kích hoạt Force Hunt reset chu kỳ kẹt cũ.
        *   Tách biệt bộ đếm di chuyển `mvpTransitCount` giúp thời gian săn boss ở mỗi map đạt độ chính xác cao.
...
*   **Nâng cấp Trình theo dõi & Nhật ký Săn Boss MVP (Mới - 2026-08-06)**:
    ...
    *   **Hiển thị Live Boss List & Chọn Boss Thủ Công**: Thêm khu vực hiển thị danh sách, HP bar, tọa độ của tất cả boss trên map hiện tại, tính khoảng cách thực tế và highlight boss mục tiêu chính. Bổ sung nút `🎯 Chọn` / `✅ Nhắm` cho phép người dùng click để chỉ định thủ công boss muốn ưu tiên săn ngay lập tức, tự động quay về chọn tự động khi boss bị diệt hoặc biến mất. Khi map trống, hiển thị thông điệp thân thiện thay vì ẩn hoàn toàn panel.
    *   **Đồng bộ Đội ngũ Săn Boss (Team Real-time Sync)**: Member tự động đồng bộ Cycle và di chuyển (warp) theo bản đồ của Leader thời gian thực. Đồng thời tự động khóa mục tiêu tấn công vào cùng một ID Boss giống Leader (bao gồm cả boss chọn thủ công), tập trung toàn bộ DPS của đội hình tiêu diệt nhanh nhất. Nhường quyền chạy chu kỳ đổi map hoàn toàn cho Leader.
