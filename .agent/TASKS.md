...
- Files related: `server.js`, `public/app.js`, `test.js`
- Acceptance criteria:
  - [x] Sửa giao diện để `"Vua sói"` nằm ở danh sách thường, `"MVP Vua sói"` nằm ở danh sách MVP.
  - [x] Sửa logic so khớp backend không dùng `.includes()` lỏng lẻo sau khi cắt chữ `"MVP"`. Dùng strict comparison so sánh trạng thái MVP của cấu hình và vật phẩm.
  - [x] Khắc phục lỗi để trống bộ lọc vẫn mua tất cả (Empty Whitelist Fallback): Nếu bật danh mục nhưng không tích chọn gì thì không quét mua bất kỳ vật phẩm nào thuộc danh mục đó.
  - [x] Bổ sung và chạy thành công unit test kiểm thử các trường hợp thường/MVP của card và egg, và test case lọc trống.
- Status: done
